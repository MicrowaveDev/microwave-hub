...
0. If the user says "production image-description queue", "image-description queue", or "image description subagents", route directly to `artist-helper` and run `npm run task:context -- artist-helper`. After that, read only `artist-helper/AGENTS.md` if the queue workflow is not already in context, then run the production wrapper it names. Do not read `portable-agent-instructions.md`, `SUBMODULES.md`, `package.json`, README files, run `npm run find:repos`, broad `rg`, or other routing probes for that workflow; `artist-helper/AGENTS.md` and the generated queue JSON are the sources of truth.
0. If the user says "production Threads post suggestions queue", "Threads post suggestions queue", or the exact prompt "Run the production Threads post suggestions queue for this week; follow the repo queue instructions, open the local review page, and hand off QA counts.", route directly to `artist-helper` and run `npm run task:context -- artist-helper`. After that, read only `artist-helper/AGENTS.md` if the manual publish queue workflow is not already in context, then follow its Manual Publish Queue Flow. Do not read `portable-agent-instructions.md`, `SUBMODULES.md`, `package.json`, README files, run `npm run find:repos`, broad `rg`, or other routing probes for that workflow.
...
- [bash/verify-backpack-core-consumers.sh](/Users/microwavedev/workspace/microwave-hub/bash/verify-backpack-core-consumers.sh): verify the same nested `backpack-game-core` commit across `mushroom-master` and `meat-master`, then run core, Mushroom, and Meat validation gates.
