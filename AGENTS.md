...
0. If the user says "production image-description queue", "image-description queue", or "image description subagents", route directly to `artist-helper` and run `npm run task:context -- artist-helper`. Do not run `npm run find:repos`, broad `rg`, or README searches for that workflow; `artist-helper/AGENTS.md` and the generated queue JSON are the sources of truth.
