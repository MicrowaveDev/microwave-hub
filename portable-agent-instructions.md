...
- Prefer `gh` for GitHub issue and PR work when it is installed. Start with `gh auth status` and fix authentication before relying on it. Useful commands: `gh issue list --search "<keywords>"`, `gh issue create --title "<title>" --body "<description>"`, `gh pr create --fill --body "<summary>\n\nCloses #123"`, and `gh pr view --web` when a browser review is useful. If `gh` is unavailable or unauthenticated and no connector can create issues/PRs, ask the user for the issue number or for permission to continue with a partial local implementation.
