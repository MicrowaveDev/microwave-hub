...
- Prefer `gh` for GitHub issue and PR work when it is installed. Start with `gh auth status`; on macOS, `gh` may store tokens in the keyring, so if sandboxed `gh auth status` reports an invalid token after the user has logged in, retry `gh auth status` or `gh api user --jq '.login'` with escalation before deciding auth is broken. Useful commands: `gh issue list --search "<keywords>"`, `gh issue create --title "<title>" --body "<description>"`, `gh pr create --fill --body "<summary>\n\nCloses #123"`, and `gh pr view --web` when a browser review is useful. If `gh` is unavailable or unauthenticated after the keyring/escalation check and no connector can create issues/PRs, ask the user for the issue number or for permission to continue with a partial local implementation.
