...
- When the user asks to make, write, or prepare a plan, create or update a durable Markdown plan file in the target repo unless they explicitly ask for a chat-only plan. Put a table of contents near the top.
- Make every plan ready for sub-agent execution: split the work into independent workstreams, name which workstreams can run in parallel, and specify each workstream's inputs, outputs, write scope, dependencies, completion criteria, and verification. Keep the original agent responsible for coordination, integration, and final handoff.
- During plan execution, original agents and sub-agents must update the plan file with implementation notes as they learn: related findings, decisions, evidence, potential issues to solve, and backlog tasks for adjacent improvements. Do not leave important discoveries only in chat or ephemeral logs.
